...
public class MementoNewsWidgetProvider extends AppWidgetProvider {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        for (int i = 0; i < appWidgetIds.length; ++i) {
            RemoteViews remoteViews = new RemoteViews(context.getPackageName(), R.layout.memento_widget_news);

            // set intent for widget service that will create the views
            Intent serviceIntent = new Intent(context, NewsListWidgetService.class);
            ...
            remoteViews.setRemoteAdapter(appWidgetIds[i], R.id.newsList, serviceIntent);
            remoteViews.setEmptyView(R.id.newsList, R.id.stackWidgetEmptyView);
            ...
            remoteViews.setPendingIntentTemplate(R.id.newsList, viewPendingIntent);
